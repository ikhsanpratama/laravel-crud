        ...
        <form method="POST"  action="{{ route('roles.store') }}" class="form-horizontal">
            ...
            <div class="card-body">
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <input class="form-control" type="text" name="name" id="name" placeholder="Nama Role" required>
                    </div>
                </div>
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        @foreach($permission as $value)
                            <label><input type="checkbox" name="permission[{{$value->id}}]" value="{{$value->id}}" class="name"> {{ $value->name }}</label><br/>
                        @endforeach
                    </div>
                </div>
            </div>
            <div class="card-footer">
                <button class="btn btn-outline-primary float-right" type="submit"><i class="fas fa-plus-square"></i> Tambah</button>
            </div> 
        </form>
        </div>
    </div>
